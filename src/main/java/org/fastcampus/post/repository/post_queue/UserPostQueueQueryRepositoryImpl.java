...
/**
 * Redis의 도입(실도입은 하지 않고, 인터페이스를 생성해 테스트상에서만 진행)으로 인해 UserPostQueueEntity는 더 이상 사용하지 않게됨.
 * 따라서 이 코드는 더 이상 사용하지 않고, 새로운 코드로 대체가 필요함. -> 이 또한 Redis를 실제 사용할 경우 구현체를 제작
 *      (현재는 테스트 더블로만 진행할 예정이라 Fake 구현체만 생성할 예정)
 * UserPostQueueCommandRepositoryImpl는 코드 일부분 수정으로 해결.
 */
